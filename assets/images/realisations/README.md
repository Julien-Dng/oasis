# Images des réalisations

Déposer ici les visuels des projets, avec **exactement** ces noms de fichiers
(le CSS `assets/styles.css` y fait référence) :

| Projet             | Carte | Fichier attendu             |
|--------------------|-------|-----------------------------|
| Smile&Pay          | w1    | `smile-and-pay.webp`        |
| Vallourec          | w2    | `vallourec.webp`            |
| Acupuncture Sanbao | w3    | `acupuncture-sanbao.png`    |
| N'oublie jamais    | w4    | `noublie-jamais.png`        |
| Maison San Tea     | w5    | `maison-san-tea.webp`       |
| ATLHES             | w6    | `athles.png`                |
| SafeTalk           | w7    | `safetalk.jpg`              |

Tant qu'un fichier est absent, la carte correspondante affiche automatiquement
son dégradé de repli — rien ne casse.

Format recommandé : WebP ou JPG optimisé, cadrage large (les cartes sont en
ratio ~4/3,2 et l'image est recadrée en `cover`, centrée).
